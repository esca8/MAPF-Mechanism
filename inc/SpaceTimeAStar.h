...
public:
    ...
    // node in the heap)
    typedef pairing_heap<
        AStarNode*, compare<LLNode::compare_node> >::handle_type open_handle_t;
    typedef pairing_heap<AStarNode*,
    ...
    AStarNode(int loc, int g_val, int h_val, LLNode* parent, int timestep,
...
public:
    ...
    pair<Path, int> findSuboptimalPath(
        ...
        double agent_w,
        bool dummy_start_node) override;  // return the path and the lowerbound
...
private:
    ...
    typedef pairing_heap<AStarNode*, compare<AStarNode::compare_node> >
        heap_open_t;
    typedef pairing_heap<AStarNode*,
                         compare<AStarNode::secondary_compare_node> >
        heap_focal_t;
